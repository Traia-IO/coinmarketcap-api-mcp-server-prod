...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_coinmarketcap_api",
      "get_coinmarketcap_api_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["coinmarketcap api", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
```python
@mcp.tool()
async def search_coinmarketcap_api(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    ...
    try:
        # The call_coinmarketcap_api_api function already has retry logic
        results = call_coinmarketcap_api_api(
...
```python
@mcp.tool()
async def get_coinmarketcap_api_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_coinmarketcap_api_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
...
- **Log retry attempts** for debugging:
  ...
  def call_coinmarketcap_api_api(...):
      # Implementation
  ```
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_coinmarketcap_api(query="example", limit=5)
   ```
